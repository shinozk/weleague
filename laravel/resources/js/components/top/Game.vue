<template>
    <div class="schedule">
        <div class="tab-category">
            <div v-for="(league, index) in leagues" :key="league.id">
                <input
                    :id="`TAB-${league.id}`"
                    type="radio"
                    name="TAB"
                    class="tab-switch"
                    :value="league.id"
                    v-model="selected"
                />
                <label
                    v-if="index == 0"
                    class="tab-label tab-left"
                    :for="`TAB-${league.id}`"
                    >{{ league.name }}</label
                >
                <label
                    v-else-if="index == league.length"
                    class="tab-label tab-right"
                    :for="`TAB-${league.id}`"
                    >{{ league.name }}</label
                >
                <label v-else class="tab-label" :for="`TAB-${league.id}`">{{
                    league.name
                }}</label>
                <div class="tab-content">
                    <div
                        v-if="selected == league.id"
                        class="schedule-sheet schedule-sheet__1"
                    >
                        <div class="more-left">
                            <span class="more-btn__left"></span>
                        </div>
                        <div class="schedule-inner">
                            <ul class="schedule-items schedule-items__1">
                                <li
                                    class="schedule-item"
                                    v-for="game in league.games"
                                    v-bind:key="game.id"
                                >
                                    <h2>{{ game.title_detail }}</h2>
                                    <p>{{ game.match_date }}</p>
                                    <div class="mach-item">
                                        <div class="mach-opponent">
                                            <img
                                                :src="game.home_team_img_url"
                                                alt=""
                                            />
                                            <p>{{ game.home_team_name }}</p>
                                        </div>
                                        <span>-</span>
                                        <div class="mach-opponent">
                                            <img
                                                :src="game.away_team_img_url"
                                                alt=""
                                            />
                                            <p>{{ game.away_team_name }}</p>
                                        </div>
                                    </div>
                                </li>
                            </ul>
                        </div>
                        <div class="more-right">
                            <span class="more-btn__right"></span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
export default {
    mounted() {
        this.selected = this.leagues[0].id;
    },
    data() {
        return {
            selected: 0,
            leagues: [
                {
                    id: 1,
                    name: "WEリーグ",
                    games: [
                        {
                            id: "20220103",
                            title_detail: "第３節",
                            match_date: "2022-09-01",
                            home_team_name: "横浜FC",
                            home_team_img_url:
                                "data:image/png;base64,",
                            away_team_name: "京都",
                            away_team_img_url:
                                "data:image/png;base64,",
                        },
                        {
                            id: "20220104",
                            title_detail: "第３節",
                            match_date: "2022-09-01",
                            home_team_name: "横浜FC",
                            home_team_img_url:
                                "data:image/png;base64,",
                            away_team_name: "京都",
                            away_team_img_url:
                                "data:image/png;base64,",
                        },
                    ],
                },
                {
                    id: 2,
                    name: "なでしこリーグ",
                    games: [
                        {
                            id: "20220103",
                            title_detail: "第３節",
                            match_date: "2022-09-01",
                            home_team_name: "横浜FC",
                            home_team_img_url:
                                "data:image/png;base64,",
                            away_team_name: "京都",
                            away_team_img_url:
                                "data:image/png;base64,",
                        },
                        {
                            id: "20220104",
                            title_detail: "第３節",
                            match_date: "2022-09-01",
                            home_team_name: "横浜FC",
                            home_team_img_url:
                                "data:image/png;base64,",
                            away_team_name: "京都",
                            away_team_img_url:
                                "data:image/png;base64,",
                        },
                    ],
                },
            ],
        };
    },
};
</script>
